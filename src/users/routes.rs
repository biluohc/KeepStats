    ...
    match state.user_query(&form.name).await {
        Ok(user) => {
            ...
            if form.verify(&user.pass) {
                let exp: DateTime<Utc> = Utc::now() + if form.rememberme { Duration::days(30) } else { Duration::hours(1) };
...
#[get("/userInfo")]
async fn user_informations(_req: HttpRequest, auth: AuthorizationService, state: AppState) -> impl Responder {
