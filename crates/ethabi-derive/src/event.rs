...
impl<'a> From<&'a ethabi::Event> for Event {
    fn from(e: &'a ethabi::Event) -> Self {
        ...
        let log_fields = names.iter().zip(kinds.iter()).map(|(param_name, kind)| quote! { pub #param_name: #kind }).collect();
        ...
        let log_init = names.iter().zip(to_log.iter()).map(|(param_name, convert)| quote! { #param_name: #convert }).collect();

        let topic_kinds: Vec<_> = e.inputs.iter().filter(|param| param.indexed).map(|param| rust_type(&param.kind)).collect();
...
impl Event {
    ...
    pub fn generate_parser(&self) -> TokenStream {
        ...
        quote! {
            if log.topics[0] == events::#name::event().signature() {
				let l = events::#name::parse_log(log)?;
                return Ok((l.name(),  json!(&l)));
            }
        }
    }

    pub fn generate_event_hash(&self) -> TokenStream {
        let name = syn::Ident::new(&self.name.to_snake_case(), Span::call_site());

        quote! {
            (stringify!(#name), events::#name::event().signature())
        }
    }
