...
impl Contract {
    ...
    pub fn generate(&self) -> TokenStream {
        ...
        let parsers: Vec<_> = self.events.iter().map(Event::generate_parser).collect();
        let event_hashs: Vec<_> = self.events.iter().map(Event::generate_event_hash).collect();
        ...
        quote! {
			...
			/// Parse log as (name, JSON)
            pub fn parse_log(log: ethabi::RawLog) -> ethabi::Result<(&'static str, serde_json::Value)> {
				use ethabi;
			...
			/// events's hash
            pub fn event_hashs() -> Vec<(&'static str, ethabi::Hash)> {
                vec![
                    #(#event_hashs),* 
                ]
            }
